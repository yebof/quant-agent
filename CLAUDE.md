...
- 配置：`config/settings.yaml` + `.env`；按 agent 独立选 OpenAI / Anthropic 模型。**2026-06-04 起所有 9 个 agent 用 OpenAI `gpt-5.5`**（5-11 曾切 claude-opus-4-7 应对 OpenAI quota，6-04 又切回 OpenAI 并升到 5.5；切 provider 一条 `sed` 命令）。Provider 路由按 model name 前缀判断：`gpt-` / `o1-` / `o3-` / `o4-` 走 OpenAI，其它走 Anthropic（`src/agents/base.py:_OPENAI_PREFIXES`）
- **跨 provider 自动 failover**：当 OpenAI(主)调用重试耗尽 / 非可重试错误(quota、死 key、宕机)后，`base.py:run()` 会**自动用 Anthropic 的 `_FALLBACK_MODEL`(=`claude-opus-4-7`)单发一次**(无重试,避免吃穿 session 窗口),成功就用它的结果继续(`AgentResult.model` 记实际用的模型,cost 按实际模型算)、失败就抛出原始 OpenAI 错误。只在「主=OpenAI 且 `.env` 有 ANTHROPIC_API_KEY」时触发;主已是 Claude 则 no-op(同 provider 无意义,且构造时不会因此报错)。截断(max_tokens)不触发 failover。pipeline 给 9 个 agent 都传 `fallback_api_key=config.api_keys.anthropic`。`src/config.py` 的 LLMConfig 默认(settings.yaml 漏配时的兜底)也已从过时的 `*-4-6` 更到 `claude-opus-4-7`
...
- **Env vars（可选调节）**：
  ...
  - `.env` 的必需项：`OPENAI_API_KEY` (当前主 provider，gpt-5.5) / `ALPACA_API_KEY` / `ALPACA_SECRET_KEY` / `FRED_API_KEY`。**`ANTHROPIC_API_KEY` 现在是 failover key**——OpenAI 调用彻底失败时自动切到 claude-opus-4-7(见上面 failover 段),所以生产环境应保持它有值;缺它则 failover 静默禁用(主调用失败直接抛错)
