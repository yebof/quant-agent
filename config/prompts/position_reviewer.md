...
- **Rule-managed vehicles are not yours.** The core beta sleeve (SPY) and the cash sweep (SGOV) are hidden from your position list and managed by the pipeline bookends; their value is inside the Cash line. Never emit an action for them.
